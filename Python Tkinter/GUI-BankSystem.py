...
def write(master,name,oc,pin,add):
    if( (is_number(name)) or (is_number(oc)==0) or (is_number(pin)==0)or name=="") :
          messagebox.showinfo("Error","Invalid Credentials\nPlease try again.")
          return
    elif len(pin)<4:
          messagebox.showinfo("Error","PIN should be atleast 4 length\nPlease try again.")
          return
    elif int(oc)<1500:
          messagebox.showinfo("Error","Opning credit should be atleast 1500 for saving Account\nPlease try again.")
          return
    ...
    frec.write(str(strftime("%d-%m-%Y %H:%M:%S",gmtime()))+"\t\t\t\t"+oc+"\t\t"+oc+"\n")
    frec.close()
...
def log_in(master):
	...
	l_title.config(font=("Courier","50","bold"))
	l_title.pack(side="top")
	l1=tk.Label(loginwn,text="Enter Name:",bg='gray',fg='white',padx=30,pady=10,relief="raised")
	l1.pack(side="top")
	e1=tk.Entry(loginwn)
	e1.pack(side="top")
	l2=tk.Label(loginwn,text="Enter account number:",bg='gray',fg='white',padx=30,pady=10,relief="raised")
	l2.pack(side="top")
	e2=tk.Entry(loginwn)
	e2.pack(side="top")
	l3=tk.Label(loginwn,text="Enter your PIN:",bg='gray',fg='white',padx=30,pady=10,relief="raised")
	l3.pack(side="top")
	e3=tk.Entry(loginwn,show="*")
	e3.pack(side="top")
	b=tk.Button(loginwn,text="Submit",bg='gray',fg='white',padx=30,pady=10,command=lambda: check_log_in(loginwn,e1.get().strip(),e2.get().strip(),e3.get().strip()))
	b.pack(side="top")
	b1=tk.Button(text="HOME",relief="raised",bg='gray',fg='white',padx=30,pady=10,command=lambda: home_return(loginwn))
	b1.pack(side="top")
...
def Create():
	...
	crwn=tk.Tk()
	var = tk.IntVar()
	R1 = tk.Radiobutton(crwn, text="Option 1", variable=var, value=1,command=sel(var))
	R1.pack( anchor = tk.W )
	R2 = tk.Radiobutton(crwn, text="Option 2", variable=var, value=2,command=sel(var))
	R2.pack( anchor = tk.W )
	R3 = tk.Radiobutton(crwn, text="Option 3", variable=var, value=3,command=sel(var))
	R3.pack( anchor = tk.W)
	crwn.geometry("600x500")
	...
	l_title.config(font=("Courier","50","bold"))
	l_title.pack(side="top")
	l1=tk.Label(crwn,text="Enter Name:",bg='gray',fg='white',padx=30,pady=10,relief="raised")
	l1.pack(side="top")
	e1=tk.Entry(crwn)
	e1.pack(side="top")
	l2=tk.Label(crwn,text="Enter opening credit:",bg='gray',fg='white',padx=30,pady=10,relief="raised")
	l2.pack(side="top")
	e2=tk.Entry(crwn)
	e2.pack(side="top")
	l3=tk.Label(crwn,text="Enter desired PIN:",bg='gray',fg='white',padx=30,pady=10,relief="raised")
	l3.pack(side="top")
	e3=tk.Entry(crwn,show="*")
	e3.pack(side="top")
	l4=tk.Label(crwn,text="Enter Address:",bg='gray',fg='white',padx=30,pady=10,relief="raised")
	l4.pack(side="top")
	e4=tk.Entry(crwn)
	e4.pack(side="top")
	b=tk.Button(crwn,text="Submit",bg='gray',fg='white',padx=30,pady=10,command=lambda: write(crwn,e1.get().strip(),e2.get().strip(),e3.get().strip(),e4.get().strip()))
	b.pack(side="top")
...
def Main_Menu():
	...
	fr1=tk.Frame(rootwn)
	fr1.pack(side="top")
	bg_image = tk.PhotoImage(file ="UnitedBank.png",width=1500)
	x = tk.Label (image = bg_image)
	x.place(x=0,y=150)
	l_title=tk.Message(text="United Bank\nWelcoms You!",relief="raised",width=1000,padx=400,pady=0,fg="white",bg="gray",justify="center",anchor="center")
	l_title.config(font=("Courier","50","bold"))
	l_title.pack(side="top")

	b1=tk.Button(text="New User",bg='gray',fg='white',padx=30,pady=10,command=Create)
	b2=tk.Button(text="Login",bg='gray',fg='white',padx=30,pady=10,command=lambda: log_in(rootwn))
	b6=tk.Button(text="Quit",bg='gray',fg='white',padx=30,pady=10,command=rootwn.destroy)
	b1.place(x=200,y=225)
	b2.place(x=200,y=275)	
	b6.place(x=200,y=325)
